...
// 인플루언서 셸 + 셸이 쓸 요약값 계산.
//
// D33 — influencer/layout.tsx 본문을 그대로 옮겼다(로직 변경 없음).
// 옮긴 이유는 AdvertiserShellServer 와 같다: /day/[date] 가 두 역할 공용이라
// 어느 역할 레이아웃에도 안 들어가 셸이 없었다.
//
// ⚠️ export const dynamic 은 여기로 못 옮긴다. 세그먼트 설정은 route 파일(layout/page)에서만
//    읽히므로 컴포넌트에 적으면 조용히 무시된다. influencer/layout.tsx 에 남겨 뒀다.
//
// (아래는 셸을 레이아웃으로 올릴 때 적어 둔 기록 — 왜 화면마다 붙이면 안 되는지)
...
//   ② 그 화면들이 써 둔 lg:[.inf-pc_&]: 변형이 전부 죽은 코드였다.
...
export default async function InfluencerShellServer({ children }: { children: React.ReactNode }) {
