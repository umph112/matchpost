...
//
// ⚠️ 전제(확정된 협업)가 없으면 return 이 아니라 skip 이다. return 하면 스위트 요약에
//    ok 로 찍혀 「회귀 확인 통과」로 읽히는데 실은 한 줄도 안 재본 것이다(D30 PROMPT-3).
//    조용한 실패를 잡으려고 도는 봇이 조용히 실패하면 안 된다.
...
test('D29P2-1 · 캠페인 딜시트: 단계 이동이 DB 에 남는다', async ({ browser }) => {
  ...
  test.skip(!deal, '확정된 캠페인 협업이 없어요 — 캠페인 쪽 협업이 먼저 성립해야 합니다 (미검증)')
...
test('D29P2-2 · 권한 없는 세션이 누르면 화면에 에러가 뜬다', async ({ browser }) => {
  ...
  const { data: rows } = await sb
    ...
    .limit(1)
  const deal = rows?.[0]
  test.skip(!deal, '확정된 오픈 협업이 없어요 — 5-1 협업 성립이 먼저 성공해야 합니다 (미검증)')
