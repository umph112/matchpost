...
//   [ ] 광고주가 /advertiser/deals/{proposalId} 로 표를 연다 (8단계)
//   [ ] 단계를 넘기면 DB proposals.stage 가 실제로 바뀐다 (D23 — 화면 말고 DB)
//   [ ] 인플루언서도 같은 표를 보되 조작(정산·세무·단계·복사)은 없다
...
test('D29-3 · 광고주: 오픈 협업 딜시트가 열리고, 단계 이동이 DB에 남는다', async ({ browser }) => {
  ...
  console.log('[D29] 오픈 협업:', deal.id, '단계(전):', before)
  ...
  // 표가 떴는가 — 8단계(협의·수락·방문·원고·수정/컨펌·게재·게재뒤수정·정산)
  await expect(page.getByText('인플루언서', { exact: true }).first()).toBeVisible()
  await expect(page.getByText('/8').first(), '오픈 협업은 8단계로 연다').toBeVisible()
  // 오픈엔 복사할 캠페인 원본이 없다
...
test('D29-4 · 인플루언서: 같은 표를 보되 조작은 없다', async ({ browser }) => {
  const deal = await pickOpenDeal()
  ...
  await expect(page.getByText('/8').first(), '인플루언서도 단계를 본다').toBeVisible()
