...
## 버전 · 진행 (테스트 2.0)
- 버전 체계: **테스트 → 베타 → 정식**. 현재 = **테스트 2.0**.
- **광고주 마이페이지 PC 대시보드** 구현 중 (디자인 핸드오프 `design/mypage-pc/README.md` = 테스트 2.0 자료).
  - 구현 순서 6단계 중 **STEP 1~5 완료**, **STEP 6 남음**:
    ...
    - ⬜ **STEP 6**: "이 날짜로 대시 보내기" → 대시 생성 프리필 실제 연결(`/advertiser/messages?to=&date=`가 프리필 받게)
  - 모바일 깨짐 1차 수정(헤더 세로배치·최근캠페인 표 가로스크롤). 모바일 폴리시는 추후.
- ⚠️ **DB**: 마이그레이션 0010(`schedules` 채널·희망페이·메모) 실행 필요(안 하면 오픈 상세 일부 미표시). ⚠️ Vercel env에 네이버 키(NAVER_API_CLIENT_ID/SECRET) 추가 필요(배포판 장소검색·오픈 조인).
...
## 다음 할 일 (후보)
- 인플루언서 캠페인 상세 뷰(양면 딜시트 첫 구현) — campaigns/[id] 확장
