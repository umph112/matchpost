...
### flex 컨테이너 안의 mx-auto (고정)
- 셸의 `<main>` 은 PC 에서 `flex flex-col` 이다. 그 안의 래퍼에 `mx-auto` 가 남아 있으면
  auto 가로여백이 `align-items: stretch` 를 무력화해 **내용 폭으로 쪼그라든다**
- 그래서 PC 해제는 `max-w-none px-0` 만으로는 부족하다 — **`mx-0` 을 함께** 줘야 한다
- 모바일용 `max-w-lg mx-auto` 를 PC 에서 풀 때는 세 개가 한 벌이다:
  `lg:[.adv-pc_&]:max-w-none lg:[.adv-pc_&]:mx-0 lg:[.adv-pc_&]:px-0`
- 증상이 「갇힘」이 아니라 「가운데 좁은 기둥」으로 나타나서 폭 문제로 안 보인다
- 같은 부류가 D33 에서 네 번 나왔다 — 화면이 자기를 좁히는 것 · UA 판정 ·
  마커가 항상 켜지는 것 · 이 mx-auto. **폭이 이상하면 폭 클래스보다 판정·조상을 먼저 의심한다**

### 헤더는 셸만 그린다 (고정)
- 셸이 상단바를 **두 폭 다** 준다(모바일용·PC용 각각). 화면이 자기 헤더를 들면 이중 헤더가 된다
- 화면 제목이 필요하면 셸의 화면 이름 표에 넣는다(`InfluencerShell` 상단 `match/title` 배열)
- ⚠️ 헤더를 찾을 때 `grep "<header"` 로 찾지 말 것 — 태그가 `<nav>` 인 헤더가 있었다.
  **태그가 아니라 컴포넌트 이름으로 찾는다**
